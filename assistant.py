...
def _call_gemini(system_prompt: str, messages: list) -> str:
    ...
    # Model-agnostic call. Attempt 1 switches "thinking" off to protect the
    # token budget (correct + cheapest on gemini-2.5-flash). Attempt 2 drops
    # thinkingConfig entirely and widens the budget — newer 3.x models reject
    # thinkingBudget=0 with an HTTP 400, and that rejection is exactly what
    # used to make this tier fall back to the KB template on every question.
    # Trying both means the assistant answers whether the configured model is
    # 2.5-class or 3.x-class.
    attempts = [
    ...
    for i, gen_config in enumerate(attempts):
        try:
            ...
            print(f"[assistant] Gemini request failed (attempt {i + 1}/{len(attempts)}): {e}")
            continue
        if not r.ok:
            last_detail = f"HTTP {r.status_code}: {r.text[:300]}"
            # Surface the REAL reason (bad model, bad key, rejected config,
            # rate limit) so it can be diagnosed from the Render logs.
            print(f"[assistant] Gemini API error {r.status_code} "
                  f"(attempt {i + 1}/{len(attempts)}): {r.text[:500]}")
            continue
        text = _gemini_extract_text(r.json())
        if text:
            return text
        last_detail = "empty text (finishReason may be MAX_TOKENS/SAFETY)"
        print(f"[assistant] Gemini returned empty text (attempt {i + 1}/{len(attempts)})")
    # Both attempts failed — raise so answer() logs it and falls back.
...
def answer(history: list, lang: str = "ar") -> dict:
    ...
    if provider:
        try:
            ...
            traceback.print_exc()
    return _kb_fallback_answer(last_question, lang, provider_configured=bool(provider))
